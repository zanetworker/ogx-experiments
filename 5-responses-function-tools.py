#!/usr/bin/env python3
"""
Complete example of using function tools with Responses API in llama-stack 0.3.0

This demonstrates:
1. Defining Python functions
2. Creating function tool schemas with JSON validation for Llama Stack
3. Using the Responses API to call functions
4. Handling function execution
5. Sending results back to the model

IMPORTANT: Llama Stack Responses API uses a FLAT structure for function tools:
{
    "type": "function",
    "name": "function_name",
    "description": "...",
...
# ============================================================================
# STEP 2: Define Function Tool Schema (Llama Stack Responses API Format)
# ============================================================================
#
# NOTE: Unlike OpenAI's Chat Completions API which uses nested format:
#   {type: "function", function: {name, parameters}}
#
# Llama Stack's Responses API uses a FLAT format:
...
def main():
    ...
    print(colored("Function Tools with Responses API - llama-stack 0.3.0", "yellow", attrs=["bold"]))
    print(colored("=" * 80, "yellow"))
    ...
    try:
        final_response = client.responses.create(
            model=model_id,
            input="",  # No new input needed
            previous_response_id=response.id,  # Continue from previous response
            tool_results=tool_results,  # Send function execution results
            stream=False
        )
...
Key Format Difference from OpenAI:
  • Llama Stack Responses API uses FLAT structure: {type, name, description, parameters}
