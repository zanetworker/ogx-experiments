#!/usr/bin/env python3
"""
Agentic architectures with the Responses API.
...
def setup_client():
    ...
    return OpenAI(base_url=url, api_key="not-needed"), model
...
TOOLS = {
    "get_competitors": {
        ...
        "schema": {
            "type": "function", "name": "get_competitors", "description": "Get competitors",
    ...
    "summarize_data": {
        ...
        "schema": {
            "type": "function", "name": "summarize_data", "description": "Summarize data",
    ...
    "send_email": {
        ...
        "schema": {
            "type": "function", "name": "send_email", "description": "Send email",
            "parameters": {
                "type": "object",
...
def _extract_output(response):
    """Extract function calls and messages from a response."""
    calls = []
    messages = []
...
def pattern_b_agentic_workflow(client, model):
    ...
    for i in range(5):
        ...
        calls, messages = _extract_output(response)

        print(f"\n--- Iteration {i + 1} ---")

        if calls:
            print(f"Tool calls: {[c['name'] for c in calls]}")
            tool_outputs = _execute_calls(calls)
        else:
            tool_outputs = None
...
def pattern_c_ai_agent(client, model):
    ...
    instructions = (
        "You are an autonomous agent. Process: "
        "1. PLAN: Create steps  2. ACT: Execute with tools  "
        "3. OBSERVE: Analyze  4. REFLECT: Update plan. "
    ...
    print(f"\nGoal: {goal}")
    print(f"Budget: {max_iterations} iterations, {max_seconds}s\n")
    ...
    for i in range(max_iterations):
        ...
        if messages:
            ...
            combined = "\n".join(messages)
            if "GOAL_COMPLETE" in combined or "GOAL_FAILED" in combined:
                completed = "GOAL_COMPLETE" in combined
                print(f"\nDone in {i + 1} iterations")
                return {"pattern": "C", "iterations": i + 1, **stats, "completed": completed}

    return {"pattern": "C", "iterations": max_iterations, **stats, "completed": False}
